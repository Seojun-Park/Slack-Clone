...
class MockUserRepository {
  #data = [{ id: 1, email: 'jinchul@email.com' }];
  findOne({ where: { email } }) {
...
describe('UsersService', () => {
  ...
  it('findByEmail은 이메일을 통해 유저를 찾아야 함', () => {
    expect(service.findByEmail('jinchul@email.com')).resolves.toStrictEqual({
      email: 'jinchul@email.com',
      id: 1,
    });
  });

  it('findByEmail은 유저를 못 찾으면 null을 반환해야 함', () => {
    expect(service.findByEmail('jinchul@emil.com')).resolves.toBe(null);
  });
});
